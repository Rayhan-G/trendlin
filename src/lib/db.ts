...
export async function createPost(env: any, postData: any) {
  ...
  const result = await env.DB
    .prepare(`
      INSERT INTO posts (slug, title, excerpt, content, category, tags, featured_image, is_draft, published_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
    `)
    .bind(slug, title, excerpt || '', content, category || '', tags || '', featured_image || '', is_draft)
    .run();
...
export async function updatePost(env: any, id: number, postData: any) {
  ...
  const result = await env.DB
    .prepare(`
      ...
      WHERE id = ?
    `)
    .bind(slug, title, excerpt || '', content, category || '', tags || '', featured_image || '', is_draft, id)
    .run();
