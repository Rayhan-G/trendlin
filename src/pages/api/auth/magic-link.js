...
export default async function handler(req, res) {
  ...
  try {
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: system-ui; max-width: 500px; margin: 0 auto; padding: 40px; text-align: center;">
          <div style="background: linear-gradient(135deg, #0f172a, #1e1b4b); padding: 40px; border-radius: 24px; color: white;">
            ...
            <p style="font-size: 12px; opacity: 0.5;">This link expires in 15 minutes.</p>
          </div>
        </div>
      `,
    })
