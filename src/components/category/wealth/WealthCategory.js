// src/components/category/wealth/WealthCategory.js
...
import WealthGrid from './WealthGrid'

export default function WealthCategory() {
  ...
  const color = '#f59e0b' // Wealth gold/orange color
  ...
  const fetchPosts = async () => {
    try {
      setLoading(true)
      setError(null)
      const data = await getCategoryPosts('wealth', 100)
      ...
      const enhancedPosts = (data || []).map(post => ({
        ...
        category: post.category || 'wealth',
        date: post.published_at || post.created_at || new Date().toISOString(),
        author: post.author || 'Wealth Advisor',
        tags: post.tags || ['finance', 'wealth']
      }))
      
      setPosts(enhancedPosts)
    } catch (err) {
      console.error('Error fetching wealth posts:', err)
      setError('Failed to load wealth insights. Please try again later.')
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="loading-container">
        <div className="loading-content">
          ...
          <p className="loading-text">Loading wealth insights...</p>
  ...
  return (
    <div className="wealth-category">
      ...
      <div className="filter-section">
        <div className="filter-header">
          <div className="filter-info">
            <h3>
              {filterStats.filtered} {filterStats.filtered === 1 ? 'Wealth Insight' : 'Wealth Insights'}
            </h3>
      ...
      <style jsx>{`
        ...
        :global(html.dark) .wealth-category {
          background: #0a0a0a;
        }
