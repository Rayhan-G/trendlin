// src/components/category/lifestyle/LifestyleCategory.js
...
export default function LifestyleCategory() {
  ...
  const color = '#f97316' // Lifestyle orange color
  ...
  const fetchPosts = async () => {
    try {
      setLoading(true)
      setError(null)
      const data = await getCategoryPosts('lifestyle', 100)
      
      // Enhance posts with additional metadata
      const enhancedPosts = (data || []).map(post => ({
        ...post,
        readingTime: post.content ? Math.ceil(post.content.replace(/<[^>]*>/g, '').split(/\s+/).length / 200) : 4,
        category: post.category || 'lifestyle',
        date: post.published_at || post.created_at || new Date().toISOString(),
        author: post.author || 'Lifestyle Curator',
        tags: post.tags || ['lifestyle', 'inspiration']
      }))
      
      setPosts(enhancedPosts)
    } catch (err) {
      console.error('Error fetching lifestyle posts:', err)
      setError('Failed to load lifestyle content. Please try again later.')
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="loading-container">
        <div className="loading-content">
          ...
          <p className="loading-text">Finding your daily inspiration...</p>
    ...
    return (
      <div className="error-container">
        <div className="error-content">
          ...
          <h2>Oops! Inspiration interrupted</h2>
          <p>{error}</p>
  ...
  return (
    <div className="lifestyle-category">
      ...
      <div className="filter-section">
        <div className="filter-header">
          <div className="filter-info">
            <h3>
              {filterStats.filtered} {filterStats.filtered === 1 ? 'Inspiration' : 'Inspirations'}
            </h3>
      ...
      <style jsx>{`
        ...
        :global(html.dark) .lifestyle-category {
          background: #0a0a0a;
        }
