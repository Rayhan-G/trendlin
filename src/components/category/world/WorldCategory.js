// src/components/category/world/WorldCategory.js
...
export default function WorldCategory() {
  ...
  const color = '#06b6d4' // World cyan color
  ...
  const fetchPosts = async () => {
    try {
      setLoading(true)
      setError(null)
      const data = await getCategoryPosts('world', 100)
      ...
      const enhancedPosts = (data || []).map(post => ({
        ...
        author: post.author || 'World News Desk',
        tags: post.tags || ['global', 'news']
      }))
      
      setPosts(enhancedPosts)
    } catch (err) {
      console.error('Error fetching world news:', err)
      setError('Failed to load world news. Please try again later.')
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="loading-container">
        <div className="loading-content">
          ...
          <p className="loading-text">Loading global news...</p>
  ...
  return (
    <div className="world-category">
      ...
      <div className="filter-section">
        <div className="filter-header">
          <div className="filter-info">
            <h3>
              {filterStats.filtered} {filterStats.filtered === 1 ? 'Global Story' : 'Global Stories'}
            </h3>
      ...
      <style jsx>{`
        ...
        :global(html.dark) .world-category {
          background: #0a0a0a;
        }
