        ...
        <tr>
          <td>
            <div class="source-cell">
              ${e.logo_url?`<img src="${e.logo_url}" class="source-logo" />`:'<div class="source-logo" style="display:flex;align-items:center;justify-content:center;font-size:1.2rem;color:#9ca3af;">📄</div>'}
              <div>
          ...
          <td><span class="badge badge-category">${f(e.category_name||"Uncategorized")}</span></td>
          <td>${e.state_code?`<span class="badge badge-state">${f(e.state_code)}</span>`:'<span style="color:#9ca3af;font-size:0.75rem;">—</span>'}</td>
          <td><span class="badge badge-type ${e.source_type_actual||"master"}">${e.source_type_actual==="master"?"Universal":"State"}</span></td>
          <td>
