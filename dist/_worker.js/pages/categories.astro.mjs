...
import { g as getDB, a as prepare } from '../chunks/db_Bt5ss5wz.mjs';
/* empty css                                 */
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  let db;
  try {
    db = getDB(Astro2.locals.runtime.env);
  } catch (error) {
  ...
  if (db) {
    ...
    try {
      ...
      heroes.forEach((hero) => {
        if (hero.slug) {
          categoryHeroes[hero.slug] = hero.hero_image;
        }
      });
      console.log("\u2705 Loaded hero images for categories:", Object.keys(categoryHeroes));
    } catch (error) {
      console.error("Error fetching category heroes:", error);
    }
  }
  categories = categoriesData.map((cat) => {
    return {
      ...cat,
      hero_image: categoryHeroes[cat.slug] || null
      // Only use hero image if it exists in database
    };
  });
  function getHeroImage(category) {
    return category.hero_image || null;
  }
...
Matrix Engine v2
    ...
    return renderTemplate`<div class="category-section"${addAttribute(category.slug, "data-category")}${addAttribute(category.ringClass, "data-ring")}${addAttribute(category.slug, "key")}> <div class="view-wrapper relative bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-md transition-all duration-500 hover:shadow-xl hover:border-gray-200/80"> <!-- ========================================== --> <!-- STATE 1: SPLIT VIEW (Default - Overview) --> <!-- ========================================== --> <div class="state-split grid grid-cols-1 lg:grid-cols-12 items-stretch min-h-[380px]"> <!-- Image Side (Uses category.image - NOT hero) --> <div${addAttribute(`lg:col-span-5 relative p-6 sm:p-8 flex items-center justify-center bg-gray-50/50 ${isImageRight ? "lg:order-2 border-l border-gray-100" : "lg:order-1 border-r border-gray-100"}`, "class")}> <div class="relative w-full aspect-[4/3] lg:aspect-square max-w-sm rounded-2xl overflow-hidden shadow-inner group"> <img${addAttribute(category.image, "src")}${addAttribute(category.name, "alt")} class="w-full h-full object-cover transition-transform duration-700 ease-out scale-102 group-hover:scale-105 filter grayscale-[10%] group-hover:grayscale-0"> <div class="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent opacity-60"></div> <div class="absolute bottom-4 left-4 bg-white/90 backdrop-blur-md rounded-xl py-2 px-3 shadow-lg border border-white/40 flex items-center gap-2"> <span class="text-xl">${category.icon}</span> <span class="text-xs font-bold uppercase tracking-wider text-gray-800" style="font-family: 'Space Grotesk', sans-serif;">${category.slug.replace("-", " ")}</span> </div> </div> </div> <!-- Content Side --> <div${addAttribute(`lg:col-span-7 p-8 sm:p-12 flex flex-col justify-between ${isImageRight ? "lg:order-1" : "lg:order-2"} relative overflow-hidden bg-white`, "class")}> <div${addAttribute(`absolute -right-24 -top-24 w-48 h-48 rounded-full bg-gradient-to-br ${targetGradient} opacity-[0.03] blur-3xl pointer-events-none`, "class")}></div> <div class="relative z-10 my-auto"> <div class="flex items-center gap-4 mb-4"> <div${addAttribute(`w-2 h-8 rounded-full bg-gradient-to-b ${targetGradient}`, "class")}></div> <h2 class="text-3xl sm:text-4xl font-bold text-gray-900 tracking-tight" style="font-family: 'Space Grotesk', sans-serif;"> ${category.name} </h2> </div> <div class="flex items-center gap-3 mb-6"> <span class="text-xs font-semibold bg-gray-100 text-gray-600 px-3 py-1 rounded-md tracking-wide"> ${count} ${count === 1 ? "Article" : "Articles"} </span> </div> <p class="text-gray-500 text-base leading-relaxed max-w-xl font-normal"> ${category.description} </p> </div> <div class="pt-8 border-t border-gray-50 mt-8 flex items-center justify-between"> <button class="explore-matrix-btn inline-flex items-center gap-3 px-6 py-3 rounded-xl text-sm font-semibold text-white transition-all duration-300 hover:shadow-lg w-fit bg-gray-900 hover:bg-black group/btn"${addAttribute(category.slug, "data-category")}> <span>View Articles</span> <svg class="w-4 h-4 transition-transform duration-300 group-hover/btn:translate-x-1" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6"></path> </svg> </button> </div> </div> </div> <!-- ========================================== --> <!-- STATE 2: EXPANDED SLIDER WITH HERO SECTION --> <!-- ========================================== --> <div class="state-slider hidden flex-col transition-all duration-500"> <!-- ========================================== --> <!-- HERO SECTION - SAME FOR EVERY CATEGORY --> <!-- ========================================== --> ${heroImage ? renderTemplate`<!-- HERO WITH IMAGE -->
                  <div class="category-hero relative w-full overflow-hidden bg-gray-900"> <div class="relative h-48 sm:h-56 md:h-64 lg:h-72"> <!-- Hero Image (Wide format) --> <img${addAttribute(heroImage, "src")}${addAttribute(`${category.name} hero`, "alt")} class="w-full h-full object-cover" loading="lazy" onerror="this.style.display='none'; this.parentElement.querySelector('.hero-fallback').style.display='flex';"> <!-- Fallback if image fails --> <div class="hero-fallback absolute inset-0 hidden items-center justify-center bg-gradient-to-r from-gray-800 to-gray-900"> <div class="text-center text-white"> <span class="text-4xl block mb-2">${category.icon}</span> <span class="text-xl font-bold">${category.name}</span> </div> </div> <!-- Gradient Overlay for text readability --> <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent"></div> <!-- Hero Content Overlay --> <div class="absolute bottom-0 left-0 right-0 p-6 sm:p-8 text-white"> <div class="max-w-3xl"> <div class="flex items-center gap-3 mb-2"> <span class="text-3xl sm:text-4xl">${category.icon}</span> <h3 class="text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight" style="font-family: 'Space Grotesk', sans-serif;"> ${category.name} </h3> </div> <p class="text-sm sm:text-base text-white/90 max-w-2xl"> ${category.description} </p> <div class="flex items-center gap-3 mt-3 flex-wrap"> <span class="text-xs font-medium bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full"> ${count} ${count === 1 ? "Article" : "Articles"} </span> <span class="text-xs font-medium bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full">
🏷️ ${category.slug.replace("-", " ")} </span> <span class="text-xs font-medium bg-emerald-500/30 backdrop-blur-sm px-3 py-1 rounded-full">
✨ Custom Hero
</span> </div> </div> </div> </div> <!-- Close Button inside hero --> <button class="close-matrix-btn absolute top-4 right-4 text-white/70 hover:text-white flex items-center gap-1.5 transition-colors duration-200 px-3 py-1.5 rounded-md bg-black/30 hover:bg-black/50 backdrop-blur-sm border border-white/20"${addAttribute(category.slug, "data-category")}> <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"></path> </svg> <span class="text-xs font-medium">Close</span> </button> </div>` : renderTemplate`<!-- HERO WITHOUT IMAGE (SIMPLE HEADER) - SAME FOR ALL CATEGORIES -->
                  <div class="bg-gradient-to-r from-gray-800 to-gray-900 px-6 py-4 flex items-center justify-between"> <div class="flex items-center gap-3 text-white"> <span class="text-2xl">${category.icon}</span> <h3 class="font-bold tracking-tight text-lg sm:text-xl" style="font-family: 'Space Grotesk', sans-serif;"> ${category.name} Archive
</h3> <span class="text-xs text-gray-400 font-normal">/ ${count} entries</span> </div> <button class="close-matrix-btn text-xs text-gray-400 hover:text-white flex items-center gap-1.5 transition-colors duration-200 px-2.5 py-1 rounded-md border border-gray-700 hover:border-gray-500"${addAttribute(category.slug, "data-category")}> <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"></path> </svg>
Esc
</button> </div>`} <!-- Horizontal Scroll - Posts --> <div class="flex overflow-x-auto gap-5 p-6 sm:p-8 bg-gray-50/70 dark:bg-gray-900/30 scroll-smooth" style="scrollbar-width: none; -ms-overflow-style: none; overflow-x: auto;"> <style>
                    .flex.overflow-x-auto::-webkit-scrollbar {
                      display: none;
                    }
                  </style> ${posts.length > 0 ? posts.map((post) => renderTemplate`${renderComponent($$result2, "PostCard", $$PostCard, { "post": post })}`) : renderTemplate`<div class="w-full text-center py-12 text-gray-400 font-light text-sm"> <span class="text-4xl block mb-3">📭</span>
No publications found in this category yet.
</div>`} </div> <!-- Scroll Hint --> <div class="flex justify-center items-center gap-1 py-3 text-[10px] font-medium text-gray-400 dark:text-gray-500 tracking-widest uppercase bg-gray-50/70 dark:bg-gray-900/30 border-t border-gray-100 dark:border-gray-800"> <svg class="w-3.5 h-3.5 animate-bounce-x" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6"></path> </svg> <span>Scroll to explore articles</span> <svg class="w-3.5 h-3.5 animate-bounce-x" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6"></path> </svg> </div> </div> </div> </div>`;
  })} </div> </div> ` })} <!-- ============================================ --> <!-- GLOBAL STYLES --> <!-- ============================================ -->  <!-- ============================================ --> <!-- JAVASCRIPT - INTERACTIVITY --> <!-- ============================================ --> ${renderScript($$result, "P:/Projects/trendlin/src/pages/categories/index.astro?astro&type=script&index=0&lang.ts")}`;
