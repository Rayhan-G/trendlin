...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  try {
    ...
    if (statsResponse.ok) {
      ...
      if (data && data.success) {
        ...
        newsletterStats = {
          total: d.subscribers?.total || d.total || 0,
          active: d.subscribers?.active || d.active || 0,
          pending: d.subscribers?.pending || d.pending || 0,
          unsubscribed: d.subscribers?.unsubscribed || d.unsubscribed || 0,
          campaigns: d.campaigns?.completed || d.completed || d.total_campaigns || 0,
          openRate: d.campaigns?.avg_open_rate || d.avg_open_rate || 0,
          clickRate: d.campaigns?.avg_click_rate || d.avg_click_rate || 0
        };
      }
    }
  } catch (error) {
  ...
  try {
    ...
    if (campaignsResponse.ok) {
      ...
      if (data && data.success) {
        recentCampaigns = data.data || data.campaigns || [];
      }
    }
  } catch (error) {
...
</a> </div>`} </div> <!-- Recent Campaigns --> <div class="recent-posts-section"> <div class="section-header"> <div> <h2 class="section-title">📨 Recent Campaigns</h2> <p class="section-subtitle">Your latest newsletter campaigns</p> </div> <div class="section-actions"> <a href="/admin/newsletter/new" class="btn-new-post"> <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"> <line x1="12" y1="5" x2="12" y2="19"></line> <line x1="5" y1="12" x2="19" y2="12"></line> </svg>
New Campaign
</a> </div> </div> <div class="table-container"> ${recentCampaigns.length > 0 ? renderTemplate`<div class="table-scroll"> <table class="posts-table"> <thead> <tr> <th>Subject</th> <th class="status-col">Status</th> <th class="date-col">Sent</th> <th class="actions-col">Actions</th> </tr> </thead> <tbody> ${recentCampaigns.map((campaign) => {
    const statusClass = getStatusClass(campaign.status);
    return renderTemplate`<tr${addAttribute(campaign.id, "key")}> <td> <div class="post-title-cell"> <span class="post-title">${campaign.subject}</span> </div> </td> <td class="status-col"> <span${addAttribute(`status-badge ${statusClass}`, "class")}> <span class="status-dot"></span> ${campaign.status} </span> </td> <td class="date-col"> ${campaign.sent_at ? new Date(campaign.sent_at).toLocaleDateString("en-US", {
      month: "short",
      day: "numeric",
      year: "numeric"
    }) : campaign.scheduled_at ? "Scheduled" : "\u2014"} </td> <td class="actions-col"> <div class="action-buttons"> <a${addAttribute(`/admin/newsletter/${campaign.id}`, "href")} class="action-btn action-btn-edit"> <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"> <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path> <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path> </svg>
Edit
</a> <button class="action-btn action-btn-delete delete-campaign-btn"${addAttribute(campaign.id, "data-id")}> <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"> <polyline points="3 6 5 6 21 6"></polyline> <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path> </svg>
Delete
</button> </div> </td> </tr>`;
  })} </tbody> </table> </div>` : renderTemplate`<div class="empty-state"> <div class="empty-icon">📨</div> <p>No campaigns yet</p> <a href="/admin/newsletter/new" class="empty-action">Create your first campaign →</a> </div>`} </div> ${recentCampaigns.length > 0 && renderTemplate`<div class="view-all-container"> <a href="/admin/newsletter" class="view-all-link">
View all campaigns →
