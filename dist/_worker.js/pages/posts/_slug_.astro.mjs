...
/* empty css                                    */
import { c as createAstro, a as createComponent, g as renderComponent, f as renderScript, d as renderTemplate, m as maybeRenderHead, b as addAttribute, u as unescapeHTML } from '../../chunks/astro/server_DVHrQl8d.mjs';
...
Back to Home
    ...
    month: "long",
    day: "numeric"
  })} </time>`} <span class="text-gray-300">•</span> <span class="text-gray-400 font-light">${post.views || 0} views</span> </div> <h1 class="text-3xl sm:text-5xl font-bold text-gray-900 tracking-tight leading-tight max-w-3xl" style="font-family: 'Space Grotesk', sans-serif;"> ${post.title} </h1> ${post.excerpt && renderTemplate`<p class="text-lg text-gray-500 max-w-2xl font-light leading-relaxed"> ${post.excerpt} </p>`} </header> <!-- Hero Image --> ${post.cover_image && renderTemplate`<div class="w-full aspect-[16/9] rounded-2xl overflow-hidden border border-gray-100 shadow-sm mb-14 bg-gray-50"> <img${addAttribute(post.cover_image, "src")}${addAttribute(post.title, "alt")} class="w-full h-full object-cover"> </div>`} <!-- Two Column Layout: Sidebar (Left) + Content (Right) --> <div class="grid grid-cols-1 lg:grid-cols-12 gap-8"> <!-- Sidebar - Left (4 columns) --> <aside class="lg:col-span-4 order-1 lg:sticky lg:top-28"> <div class="space-y-6"> <!-- Table of Contents --> ${headings.length > 0 && renderTemplate`<div class="bg-gray-50 dark:bg-gray-800/30 rounded-2xl p-6 border border-gray-100 dark:border-gray-700/50"> <span class="text-[10px] font-bold uppercase tracking-widest text-gray-400 block mb-4" style="font-family: 'Space Grotesk', sans-serif;">
On this page
