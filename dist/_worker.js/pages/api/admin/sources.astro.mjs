...
async function POST({ request, locals }) {
  try {
    ...
    const newSource = await DB.prepare("SELECT * FROM sources WHERE id = ?").bind(result.meta.last_row_id).first();
    return new Response(JSON.stringify({
      success: true,
      source: newSource
    }), {
...
async function PUT({ request, locals }) {
  try {
    ...
    await DB.prepare(`
      ...
      WHERE id = ?
    `).bind(name, url, category, description || "", source_type || "official", logo_url || "", is_active !== void 0 ? is_active : 1, id).run();
    const updatedSource = await DB.prepare("SELECT * FROM sources WHERE id = ?").bind(id).first();
    return new Response(JSON.stringify({
      success: true,
