...
export default function UnifiedHero() {
  ...
  // Design dimensions — this is your locked canvas size
  ...
  // Handle scaling — no scrollbars
  useEffect(() => {
    function updateScaleAndHeight() {
      const viewportWidth = window.innerWidth
      const viewportHeight = window.innerHeight
      
      // Get navbar height (adjust based on your actual navbar)
      const navbar = document.querySelector('nav')
      const navbarHeight = navbar ? navbar.offsetHeight : 64
      
      // Available height for hero
      const availableHeight = viewportHeight - navbarHeight
      
      // Calculate scale based on width (primary)
      let newScale = viewportWidth / DESIGN_WIDTH
      newScale = Math.min(newScale, 1.0) // Never grow past original
      
      // Check if scaled height exceeds available height
      let scaledHeight = DESIGN_HEIGHT * newScale
      
      // If height is too tall, scale down based on height instead
      if (scaledHeight > availableHeight && availableHeight > 0) {
        newScale = availableHeight / DESIGN_HEIGHT
      }
      
      setScale(newScale)
      
      // Set wrapper height to exactly match scaled hero
      const finalHeight = DESIGN_HEIGHT * newScale
    ...
    window.addEventListener('orientationchange', updateScaleAndHeight)
    
    return () => {
      window.removeEventListener('resize', updateScaleAndHeight)
      window.removeEventListener('orientationchange', updateScaleAndHeight)
    }
  }, [])
        ...
        if (!categoryError && categoryData && categoryData.length > 0) {
          ...
          if (!postsError && posts && posts.length > 0) {
            ...
            posts.forEach(post => {
              ...
              catStats.total_posts++
              if (post.total_ratings && post.total_ratings > 0) {
                catStats.total_ratings += post.total_ratings
                catStats.weighted_rating += (post.average_rating || 0) * post.total_ratings
              }
            })
  ...
  return (
    <>
      {/* Wrapper with no overflow - NO SCROLLBARS */}
      <div 
        ref={wrapperRef}
        style={{
          width: '100%',
      ...
      >
        {/* The locked hero — scales uniformly like an image */}
        <div 
          ref={heroRef}
          className={`hero locked-hero ${isDarkMode ? 'dark' : 'light'}`}
          style={{
      ...
      <style jsx>{`
        ...
        .locked-hero.dark .gradient-1 {
          background: radial-gradient(circle, rgba(139,92,246,0.3), transparent);
        }

        .locked-hero.light .gradient-1 {
        ...
        .locked-hero.dark .gradient-3 {
          background: radial-gradient(circle, rgba(6,182,212,0.2), transparent);
        }

        .locked-hero.light .gradient-3 {
        ...
        .locked-hero.dark .left {
          --text-primary: white;
        }

        .locked-hero.light .left {
          --text-primary: #111827;
        }
        ...
        .locked-hero.dark .eyebrow {
          background: rgba(255,255,255,0.05);
          border: 1px solid rgba(255,255,255,0.1);
          color: rgba(255,255,255,0.7);
        }

        .locked-hero.light .eyebrow {
        ...
        .locked-hero.dark .description {
          color: rgba(255,255,255,0.6);
        }

        .locked-hero.light .description {
          color: rgba(0,0,0,0.6);
        }
        ...
        .locked-hero.dark .stat-number {
          color: white;
        }

        .locked-hero.light .stat-number {
          color: #111827;
        }
        ...
        .locked-hero.dark .stat-label {
          color: rgba(255,255,255,0.5);
        }

        .locked-hero.light .stat-label {
        ...
        .locked-hero.dark .stat-note {
          color: rgba(255,255,255,0.3);
        }

        .locked-hero.light .stat-note {
        ...
        .locked-hero.dark .stat-divider {
          background: rgba(255,255,255,0.1);
        }

        .locked-hero.light .stat-divider {
        ...
        .locked-hero.dark .orb {
          box-shadow: 0 0 70px rgba(139,92,246,0.5);
        }

        .locked-hero.light .orb {
        ...
        .locked-hero.dark .ring {
          border: 1px solid rgba(139,92,246,0.3);
        }

        .locked-hero.light .ring {
        ...
        .locked-hero.dark .icons span {
          text-shadow: 0 0 15px rgba(139,92,246,0.3);
        }

        .locked-hero.light .icons span {
        ...
        .locked-hero.dark .scroll-hint span {
          color: white;
        }

        .locked-hero.light .scroll-hint span {
          color: #111827;
        }
        ...
        .locked-hero.dark .scroll-line {
          background: linear-gradient(180deg, white, transparent);
        }

        .locked-hero.light .scroll-line {
